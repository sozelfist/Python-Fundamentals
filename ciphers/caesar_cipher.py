...
def caesar_cipher(text: str, key: int, encrypt: bool = True) -> str:
    ...
    for char in text:
        ...
        code = ord(char)
        # Shift the code by the key (positive for encryption, negative for decryption)
        shift = key if encrypt else -key
        new_code = (code - 65 + shift) % 26 + 65 if char.isupper() \
            else (code - 97 + shift) % 26 + 97 if char.islower() else code
...
class TestCaesarCipher(unittest.TestCase):
    ...
    def test_encrypt(self):
        ...
        self.assertEqual(caesar_cipher("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 25), "ZABCDEFGHIJKLMNOPQRSTUVWXY")
    ...
    def test_decrypt(self):
        ...
        self.assertEqual(caesar_cipher("WFAOVU PZ NYLHA", 7, encrypt=False), "PYTHON IS GREAT")
        self.assertEqual(caesar_cipher("ZABCDEFGHIJKLMNOPQRSTUVWXY", 25, encrypt=False), "ABCDEFGHIJKLMNOPQRSTUVWXYZ")
